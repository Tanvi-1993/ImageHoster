...
public class UserController {
	...
	public String registerUser(User user, Model model) {
		...
		String pwdError = "Password must contain atleast 1 alphabet, 1 number & 1 special character";

		String[] verifyExpression = { ".*[a-zA-Z]+.*", ".*[0-9]+.*", ".*[!@#$%^&*(),.?:{}<>+]|.*" };

		if ((password.matches(verifyExpression[0])
				&& (password.matches(verifyExpression[1]) && password.matches(verifyExpression[2])))) {
			userService.registerUser(user);
			return "users/login";
		} else {
			model.addAttribute("User", user);
			model.addAttribute("passwordTypeError", pwdError);
			return "users/registration";
