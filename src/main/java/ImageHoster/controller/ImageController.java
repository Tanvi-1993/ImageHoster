...
public class ImageController {
	...
	// getImage() method in the business logic is to fetch all the details of that
	// image
	...
	// Converts the image to Base64 format and store it as a string in the
	// 'imageFile' attribute
	...
	public String editImage(@RequestParam("imageId") Integer imageId, Model model, HttpSession session) {
		...
		Boolean flag = userValidation(image.getUser(), session);
		if (flag) {
	...
	public String editImageSubmit(@RequestParam("file") MultipartFile file, @RequestParam("imageId") Integer imageId,
			@RequestParam("tags") String tags, Image updatedImage, HttpSession session, Model model)
			throws IOException {
		Image image = imageService.getImage(imageId);
